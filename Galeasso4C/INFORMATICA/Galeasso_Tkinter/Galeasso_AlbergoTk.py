#GALEASSO FEDERICO 22/3/2023 COMPITO - ALBERGO TKINTER
...
class Finestra(tkinter.Tk):
	...
	def crea_widgets(self):
		...
		def calcola():
			...
			if len(nome_val)==0 or len(cognome_val)==0:
				messagebox.showerror("Errore", "Compilare tutti i campi")
			else:
				if nTotPers_val == (nAdu_val+nBam_val) and nNot_val > 0:
				...
				if nTotPers_val != (nAdu_val+nBam_val):
					 ...
					 entry_nTotPers.delete(0,1000)
					 entry_nAdu.delete(0,1000)
					 entry_nBam.delete(0,1000)
				else:
		...
		def annulla():
			...
			entry_nBam.delete(0,1000)
			entry_nNot.delete(0,1000)
			entry_somDaVers.delete(0,1000)
			entry_nCamSing.delete(0,1000)
			entry_nCamMat.delete(0,1000)
			entry_nome.delete(0,1000)
			entry_cognome.delete(0,1000)
			entry_nTotPers.configure(bg="white")
			entry_nNot.configure(bg="white")
		...
		entry_somDaVers=tkinter.Entry(mf, textvariable=somDaVers, state='readonly')
