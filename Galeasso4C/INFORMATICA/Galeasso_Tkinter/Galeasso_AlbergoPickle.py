#GALEASSO FEDERICO 23/3/2023 COMPITO - ALBERGO TKINTER - PICKLE
#Usando la tecnica del pickling in python,  memorizzare le informazione della prenotazione effettuata in un file binario per poterla richiamare alla successiva apertura del programma di prenotazione
	...
	def crea_widgets(self):
		...
		def salva_prenotazione():
			prenotazione = {
				"nome": nome.get(),
				"cognome": cognome.get(),
				"nTotPers": nTotPers.get(),
				"nAdu": nAdu.get(),
				"nBam": nBam.get(),
				"nNot": nNot.get(),
				"nCamSing": nCamSing.get(),
				"nCamMat": nCamMat.get(),
			}

			with open("prenotazione.txt", "wb") as fh:
				pickle.dump(prenotazione, fh)
						
		def carica_prenotazione():
			with open("prenotazione.txt", "rb") as fh:
				prenotazione = pickle.load(fh)

			entry_nome.insert(0,prenotazione["nome"])
			entry_cognome.insert(0,prenotazione["cognome"])
