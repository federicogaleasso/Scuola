...
io.on('connection', (socket) => {
	...
	socket.on('inviaProdotto', (data) => {
		const { nick, nome, prezzo, quantita, ubicazione } = data;
		// Aggiunge le informazioni del prodotto al magazzino e lo invia a tutti i client
		magazzino.push({ nick, nome, prezzo, quantita, ubicazione });
		io.emit('elencomagazzino', magazzino);
	}); 

	// Gestione dell'evento 'rimuoviProdotto' (quando un client rimuove un prodotto)
	socket.on('rimuoviProdotto', (data) => {
		console.log(data)
		for(let i=0;i<magazzino.length;i++){
			if(magazzino[i].nome == data){
				// console.log("...")
	...
	// Gestione dell'evento 'cercaProdotto' (quando un client ricerca di un prodotto)
	socket.on('cercaiProdotto', (data) => {
		const { nome } = data;
	});
});