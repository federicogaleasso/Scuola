...
class Finestra(tkinter.Tk):
	...
	def crea_widgets(self):
		...
		entry=tkinter.Entry(mf, textvariable=nome)
		...
		entry2=tkinter.Entry(mf, textvariable=cognome)
		...
		def stampa():
			nome_lbl=tkinter.Label(mf, text="Nome: ")
			nome_lbl.grid(row=6, column=0)
			nome_grid=tkinter.Label(mf, textvariable=nome)
			nome_grid.grid(row=6, column=1)
			
			cognome_lbl=tkinter.Label(mf, text="Cognome: ")
			cognome_lbl.grid(row=7, column=0)
			cognome_grid=tkinter.Label(mf, textvariable=cognome)
			cognome_grid.grid(row=7, column=1)
			
		
		#Invia
		btn1=tkinter.Button(mf, text="Invia", command=stampa)
